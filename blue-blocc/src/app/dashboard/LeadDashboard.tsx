...
export default function LeadDashboard() {
  ...
  const load = async () => {
    ...
    setTreso(t); setDemandes(d); setItems(i)
        ...
        {showNewItem && (
          <div className="mb-4 p-4 rounded-lg" style={{ background: 'rgba(30,107,255,0.08)', border: '1px solid rgba(30,107,255,0.2)' }}>
            <div className="grid grid-cols-1 md:grid-cols-[1fr_130px_90px_auto_auto] gap-3">
              ...
              <button className="btn-primary py-2" onClick={async () => { if (!newItemNom || !newItemPrix) return; await createItem(newItemNom.trim(), Number(newItemPrix), newItemUnite); setNewItemNom(''); setNewItemPrix(''); setNewItemUnite('kg'); setNewItemPrixVente(''); setShowNewItem(false); load() }}><Check size={15} /></button>
            ...
            <div key={item.id} className="rounded-lg px-4 py-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--blocc-border)' }}>
              {editItemId === item.id ? (
                ...
                <div className="flex items-center justify-between">
                  <div><span className="font-semibold text-white text-sm">{item.nom}</span><span className="text-xs ml-3" style={{ color: 'var(--blocc-muted)' }}>Prix réf: {formatMoney(item.prixAchat)}/{item.unite}</span></div>
                  <div className="flex gap-2">
